...
export const LeaderboardCard: React.FC<LeaderboardCardProps> = ({ currentUsername }) => {
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(apiUrl('/api/submissions/leaderboard'));
      if (!res.ok) {
    ...
    <div className="bg-gray-900/70 rounded-2xl p-4 border border-gray-800 shadow-xl mb-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[10px] text-gray-400">Ranked by unique DSA problems solved</span>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="p-6 text-center text-gray-400 text-xs bg-gray-800/20 border border-gray-800/40 rounded-xl">
          ...
          <p className="font-semibold text-gray-300">No submissions recorded yet</p>
        ...
        <div className="space-y-1.5">
              ...
              >
                ...
                <div className="flex items-center gap-2.5 min-w-0 flex-1 mr-2">
                  ...
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    {isCurrentUser && (
                      <span className="shrink-0 text-[9px] font-extrabold uppercase px-1.5 py-0.2 rounded bg-yellow-400/20 text-yellow-300 border border-yellow-400/30">
                        You
                      </span>
                    )}
                  </div>
                ...
                {/* Right: Solved + Difficulty breakdown */}
                <div className="flex items-center gap-2 shrink-0">
                  {/* Difficulty Breakdown badges */}
                  <div className="hidden sm:flex items-center gap-1 text-[10px] font-mono">
                    <span className="text-emerald-400 font-semibold" title={`${user.easy} Easy solved`}>
                      {user.easy}E
                    </span>
                    <span className="text-gray-500">•</span>
                    <span className="text-amber-400 font-semibold" title={`${user.medium} Medium solved`}>
                      {user.medium}M
                    </span>
                    <span className="text-gray-500">•</span>
                    <span className="text-rose-400 font-semibold" title={`${user.hard} Hard solved`}>
                      {user.hard}H
                    </span>
                  </div>
          ...
          {/* Show More / Show Less Button if > 5 users */}
