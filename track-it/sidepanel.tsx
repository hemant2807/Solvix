...
import {
  getSheetById,
  DEFAULT_SHEET_ID,
  findDifficultyByQuestionName,
} from "./data/sheets";
import {
  getSimilarQuestions,
  type AlternativeProblem,
} from "./data/premiumAlternatives";
...
function IndexSidePanel() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "profile" | "stats" | "practice" | "ai"
  >("dashboard");
  ...
  const [currentProblemSlug, setCurrentProblemSlug] = useState<string | null>(
    null,
  );
  const [premiumAlternatives, setPremiumAlternatives] = useState<
    AlternativeProblem[]
  >([]);
  ...
  const [githubRepos, setGithubRepos] = useState<
    Array<{ name: string; fullName: string; private: boolean }>
  >([]);
  const [githubBranches, setGithubBranches] = useState<string[]>([]);
  const [githubLoading, setGithubLoading] = useState(false);
  const [githubError, setGithubError] = useState("");
  const [githubSuccess, setGithubSuccess] = useState("");
  const [githubAuthStep, setGithubAuthStep] = useState<
    "idle" | "oauth" | "select_repo"
  >("idle");
  ...
  useEffect(() => {
    let cancelled = false;
    readChromeStorage<number>(CHROME_STORAGE_KEYS.DAILY_GOAL, 3).then(
      (value) => {
        if (!cancelled) setDailyGoal(value);
      },
    );
    const unsubscribe = onChromeStorageKeyChanged<number>(
      CHROME_STORAGE_KEYS.DAILY_GOAL,
      (value) => {
  ...
  useEffect(() => {
    if (activeTab !== "profile" || !leetcodeUser?.username || profileSummary)
      return;

    const loadProfileSummary = async () => {
      const sheetId = await readChromeStorage<string>(
        CHROME_STORAGE_KEYS.SELECTED_SHEET,
        DEFAULT_SHEET_ID,
      );
      ...
      try {
        const submissionsRes = await fetch(
          apiUrl(`/api/submissions/user/${leetcodeUser.username}`),
        );
        if (submissionsRes.ok) {
          ...
          if (totalSolved > 0) {
            ...
            favoriteLanguage =
              languageStats.find((l) => l.language !== "Unknown")?.language ||
              "—";
            ...
            const last14Days = submissions.filter(
              (s) =>
                Date.now() - new Date(s.submittedAt).getTime() < 14 * 86400000,
            );
    ...
    const loadGithubStatus = async () => {
      try {
        const res = await fetch(
          apiUrl(`/api/github/status/${leetcodeUser.username}`),
        );
        if (!res.ok) return;
  ...
  }, [prev, title, elapsed, frozenElapsed, attempts, timerStopped]);
  ...
  const loadGithubRepos = async () => {
    ...
    try {
      const res = await fetch(
        apiUrl(`/api/github/repos/${leetcodeUser.username}`),
      );
      const data = await res.json();
  ...
  const loadGithubBranches = async (repo: string) => {
    ...
    try {
      const res = await fetch(
        apiUrl(
          `/api/github/branches/${leetcodeUser.username}?repo=${encodeURIComponent(repo)}`,
        ),
      );
      const data = await res.json();
      if (res.ok && Array.isArray(data)) {
  ...
  const connectGitHub = async () => {
    ...
    if (typeof chrome === "undefined" || !chrome.identity) {
      setGithubError(
        "GitHub connect requires the Chrome extension environment. Please reload.",
      );
      return;
    }
    ...
    try {
      ...
      if (!authUrlRes.ok || !authUrlData.url) {
        setGithubError(
          authUrlData.error || "GitHub OAuth is not configured on the server.",
        );
        return;
      }
      ...
      if (!exchangeRes.ok || !exchangeData.accessToken) {
        setGithubError(
          exchangeData.error || "Failed to exchange GitHub authorization code.",
        );
        return;
      }
      ...
      if (!connectRes.ok || !connectData.success) {
        setGithubError(
          connectData.error || "Failed to save GitHub connection.",
        );
        return;
      }
      ...
      setGithubError(
        error?.message || "GitHub connection failed. Please try again.",
      );
      setGithubAuthStep("idle");
    } finally {
  ...
  const checkLoginWithTimeout = async (
    timeoutMs = 15000,
    intervalMs = 1000,
  ) => {
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-800/30 backdrop-blur-md rounded-2xl border border-gray-700/50 shadow-2xl hover:shadow-yellow-500/10 transition-all duration-500 hover:scale-[1.02]">
                ...
                <div className="flex-1">
                  <h2 className="text-lg font-bold text-white flex items-center gap-2 group">
                    ...
                    <Trophy
                      className="text-yellow-400 group-hover:animate-bounce transition-all duration-300"
                      size={16}
                    />
                  </h2>
                ...
                {activeTab === "profile" && (
                  <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-bold text-white">
                            Connected: {leetcodeUser.username}
                          </p>
                          <p className="text-[10px] text-gray-500">
                            Ranking #{leetcodeUser.ranking.toLocaleString()}
                            {leetcodeUser.contestRating
                              ? ` • Contest ${leetcodeUser.contestRating}`
                              : ""}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      ...
                      <div className="bg-gray-900/60 p-3 rounded-2xl border border-gray-800 text-center">
                        <div className="text-[10px] text-green-400 uppercase font-bold mb-1">
                          Solved
                        </div>
                        <div className="text-lg font-extrabold text-white">
                      ...
                      <div className="bg-gray-900/60 p-3 rounded-2xl border border-gray-800 text-center">
                        <div className="text-[10px] text-purple-400 uppercase font-bold mb-1">
                          Sheet
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-2.5">
                      <div className="bg-gray-900/60 p-3 rounded-2xl border border-gray-800 text-center">
                        <div className="text-[10px] text-orange-400 uppercase font-bold mb-1">
                          Longest Streak
                        </div>
                        <div className="text-lg font-extrabold text-white">
                          {profileSummary
                            ? `${profileSummary.longestStreak}d`
                            : "—"}
                        </div>
                      </div>
                      <div className="bg-gray-900/60 p-3 rounded-2xl border border-gray-800 text-center">
                        ...
                        <div className="text-lg font-extrabold text-white">
                          {profileSummary
                            ? formatDuration(profileSummary.avgTime)
                            : "—"}
                        </div>
                      </div>
                      <div className="bg-gray-900/60 p-3 rounded-2xl border border-gray-800 text-center">
                        <div className="text-[10px] text-pink-400 uppercase font-bold mb-1">
                          Favorite Topic
                        </div>
                        <div className="text-xs font-bold text-white truncate">
                      ...
                      <div className="bg-gray-900/60 p-3 rounded-2xl border border-gray-800 text-center">
                        ...
                        <div className="text-xs font-bold text-white truncate">
                          {profileSummary
                            ? profileSummary.favoriteLanguage
                            : "—"}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="bg-gray-900/60 rounded-2xl p-3 border border-gray-800">
                        ...
                        <div className="flex gap-1">
                          {Array.from({ length: 14 }).map((_, i) => {
                            ...
                            const day = profileSummary.calendarPreview.find(
                              (d) => d.date === iso,
                            );
                            const intensity = !day
                              ? "bg-gray-800"
                            ...
                            return (
                              <div
                                key={iso}
                                title={`${iso}: ${day?.count || 0} solved`}
                                className={`flex-1 h-4 rounded-sm ${intensity}`}
                              />
                            );
                          })}
                        </div>
                    ...
                    <div className="bg-gray-900/60 rounded-2xl p-3.5 border border-gray-800">
                      ...
                      {!profileSummary ? (
                        ...
                        <p className="text-xs text-gray-500 text-center py-2">
                          No submissions yet — solve a problem on LeetCode to
                          see it here.
                        </p>
                      ) : (
                      ...
                      {!githubConnected ? (
                        ...
                        <div className="space-y-2">
                          <p className="text-[10px] text-gray-500">
                            ...
                            . Select a repository to sync solutions to:
                          </p>
                          <select
                            value={githubRepo}
                            onChange={(e) => {
                          ...
                          {githubRepo && (
                            <>
                              ...
                              <button
                                onClick={() =>
                                  selectGithubRepo(githubRepo, githubBranch)
                                }
                                disabled={githubLoading}
